...
The point occurrence data were taken as point coordinates of samples. For trawled samples, their point coordinates were taken as the mid-latitude and mid-longitude and were calculated by taking the mean of the start and end latitudes/longitudes of the trawl where the specimen was collected.
...
# Visualise
mapview(seasnake_sf, col.regions = "orangered", alpha = 0, cex = 5, legend = F) +
  mapview(nw_shelf, alpha.regions = 0, legend = F)
```
...
# Visualise
mapview(seasnake_trn, color = "orangered", legend = F)
```
...
One way to look at bias layer (from: [*https://scottrinnan.wordpress.com/2015/08/31/how-to-construct-a-bias-file-with-r-for-use-in-maxent-modeling/*](https://scottrinnan.wordpress.com/2015/08/31/how-to-construct-a-bias-file-with-r-for-use-in-maxent-modeling/){.uri})
...
mapview(effort_nwshelf_sf, col.regions = "orangered", alpha = 0, cex = 5, legend = F) +
  mapview(nw_shelf, alpha.regions = 0, legend = F)
```
...
mapview(ss_effort_pts_bias) +
  mapview(nw_shelf, alpha.region = 0, legend = F)
```
