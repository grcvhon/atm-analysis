## Determine samples of <i>Aipysurus apraefrontalis</i> and <i>A. foliosquama</i> with RADseq data and prepare sample sheet for genomic analyses
...
To check, this command was used: `paste -d, sample-sheet.csv barcodes.csv | awk '{ if ($2 == $4) print "yes" }'` which prints "yes" if `dart_id` ($2) in `sample-sheet.csv` matches with `targetid` in `barcodes.csv` ($4).<br>

### Finalise the `sample sheet.csv` file in the desired format
In the beginning, we 
paste -d, sample-sheet.csv barcodes.csv