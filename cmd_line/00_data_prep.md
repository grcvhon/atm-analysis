...
To check, this command was used: `paste -d, sample-sheet.csv barcodes.csv | awk '{ if ($2 == $4) print "yes" }'` which prints "yes" if `dart_id` ($2) in `sample-sheet.csv` matches with `targetid` in `barcodes.csv` ($4).
