...
prep_input <- function(keep_nw_vcf, species_nw){
  ...
  message("Loading MARSPEC layers (annual)...")
  ...
  species_keep_nw_coords <- species_nw %>% 
    # arrange `species_keep_nw_coords` as in samples in VCF file
  ...
  message("Acquiring matrix of ancestry coefficients...")
  species_qmat <- tess3r::qmatrix(tess3 = species_tessobj, K = species_bestK)
  
  # sample coordinates
  message("Loading sample coordinates...")
  ...
  # write output as csv
  species_out <- raster::raster(species_krig_sh_admix$K1) # takes K=1
  species_out <- terra::rast(species_out)
  species_out <- terra::project(species_out, "+proj=longlat +datum=WGS84")
  df_species_out <- as.data.frame(species_out, xy = TRUE)
  # set write dir
  setwd("C:/Users/a1235304/Dropbox/Short-nosed and Leaf-scaled sea snake TSSC/atm-analysis/ancestry_layer/k-layer/")
  write.csv(df_species_out, file = paste0(species_nw,"_K1.csv"), append = FALSE)
