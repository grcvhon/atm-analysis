...
### ***                Speed                *** ###
### *** lines for bearing are commented out *** ###
...
system.time( # Keep track of how long this takes
  for (i in 1:nrow(ecoflow_pts_comb)) {           
    ...
    ecoflow_speed_passages[[i]] <- passage(sws_layer_tr,                        # run the passage function 
...
ecoflow_speed_passages <- stack(ecoflow_speed_passages) # create a raster stack of all the passage probabilities
ecoflow_speed_passages_overlay <- sum(ecoflow_speed_passages)/nrow(ecoflow_pts_comb) # calculate average



### *** save output *** ###

dir <- paste0("/hpcfs/users/a1235304/atm_passage/output/sbs_speed_seed",seed,"_",n_base,"pts_",format(Sys.time(),"%Hh%Mm%Ss"),"/")
dir.create(dir, recursive = TRUE)

# as csv
library(terra)
ovr <- rast(ecoflow_speed_passages_overlay)
df_ovr <- as.data.frame(ovr, xy = TRUE)

library(utils)
write.csv(df_ovr, file = paste0(dir, "sbs_speed_seed",seed,"_",n_base,"pts_",format(Sys.time(),"%Hh%Mm%Ss"),".csv"))
          
# as pdf

pdf(file = paste0(dir, "sbs_speed_seed",seed,"_",n_base,"pts_",format(Sys.time(),"%Hh%Mm%Ss"),".pdf"), height = 8.5, width = 11)

colors <- c("grey50", viridis_pal(option="inferno", begin = 0.3, end = 1)(20))
ggplot(as.data.frame(ecoflow_speed_passages_overlay, xy=T)) +
