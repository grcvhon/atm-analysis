...
dir <- paste0("/hpcfs/users/a1235304/atm_passage/output/regular_seed",seed,"_",size,"pts_",format(Sys.time(), "%Hh%Mm%Ss"),"/")
dir.create(dir, recursive = TRUE)

# as csv
library(terra)
ovr <- rast(reg_bearing_passages_overlay)
df_ovr <- as.data.frame(ovr, xy = TRUE)

library(utils)
write.csv(df_ovr, file = paste0(dir, "regular_seed",seed,"_",size,"pts_",format(Sys.time(), "%Hh%Mm%Ss"),".csv"))

# as pdf

pdf(file = paste0(dir, "regular_seed",seed,"_",size,"pts_",format(Sys.time(), "%Hh%Mm%Ss"),".pdf"), height = 8.5, width = 11)
